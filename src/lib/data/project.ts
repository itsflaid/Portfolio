...
export const projects: Project[] = [
	{
		...
		caseStudy: {
			problem: {
				en: 'While building DevNote (project 02), growing reliance on AI during development exposed a gap: as the codebase grew, the mental model of how everything connected got harder to hold onto. The same pattern showed up industry-wide — AI agents have to rebuild an understanding of a codebase every new session, so context rarely carries over. Tools like Claude\u2019s CLAUDE.md or Cursor\u2019s indexing help, but each lives inside its own ecosystem. There was room for something that understood a codebase directly and produced context any agent could reuse.',
				id: 'Saat mengerjakan DevNote (project 02), ketergantungan pada AI selama development mulai memperlihatkan masalah — codebase yang makin kompleks bikin sulit menjaga gambaran utuh soal bagaimana tiap bagian saling terhubung. Pola yang sama juga terlihat lebih luas: AI agent harus membangun ulang pemahaman terhadap codebase di setiap sesi baru, sehingga konteks jarang bisa dipakai ulang secara konsisten. Tools seperti CLAUDE.md di Claude atau indexing di Cursor membantu, tapi masing-masing terkunci di ekosistemnya sendiri. Ada ruang untuk solusi yang memahami codebase secara langsung sekaligus menghasilkan konteks yang bisa dipakai lintas agent.'
			},
			solution: {
				en: 'DevMap sits as a context layer between the codebase and the AI agent. It runs static analysis to map structure, file relationships, dependencies, and key components, then turns that into structured, reusable context — so neither developers nor agents have to rebuild understanding from scratch each session. Because it isn\u2019t tied to any single agent or platform, that context can move across different AI-assisted workflows instead of staying locked to one tool.',
				id: 'DevMap berperan sebagai context layer di antara codebase dan AI agent. Ia menjalankan static analysis untuk memetakan struktur, hubungan antar-file, dependency, dan komponen penting, lalu mengubahnya jadi konteks terstruktur yang reusable — jadi developer maupun agent tidak perlu membangun ulang pemahaman dari nol setiap sesi. Karena tidak terikat pada satu agent atau platform, konteks tersebut bisa dipakai di berbagai workflow AI-assisted development, bukan cuma satu tool tertentu.'
			}
		}
	},
	{
		...
		caseStudy: {
			problem: {
				en: 'Building ChatMe and DailyFit on similar stacks surfaced a recurring habit: setup steps, snippets, and files kept getting reused across projects, usually by digging back through old repos to copy-paste them. That called for one place to store and manage anything worth reusing. The need turned out to be broader than personal — when a classmate missed a lecture and needed code, sharing a single file worked fine, but sharing several at once quickly became impractical. That gap became Workspace: a way to group and share multiple files together.',
				id: 'Mengerjakan ChatMe dan DailyFit (project 03 & 04) dengan stack yang mirip memunculkan kebiasaan berulang — setup, snippet, dan file tertentu terus dipakai ulang antar-project, biasanya dengan membuka repo lama untuk copy-paste. Itu jadi alasan untuk punya satu tempat khusus menyimpan dan mengelola apa pun yang layak dipakai ulang. Kebutuhannya ternyata lebih luas dari sekadar personal — ketika teman kampus ketinggalan materi dan butuh kode, berbagi satu file saja gampang, tapi berbagi beberapa file sekaligus jadi kurang praktis. Dari situ muncul Workspace: cara mengelompokkan dan membagikan beberapa file sekaligus.'
			},
			solution: {
	...
	},
	{
		...
		caseStudy: {
			problem: {
				en: 'WhatsApp had become the default place to note down almost everything — tasks, errands for the family store, or ideas that showed up out of nowhere. It worked, but WhatsApp was never built for managing notes and reminders, and information got harder to sort through as it piled up.',
				id: 'WhatsApp jadi tempat default mencatat hampir semua hal — tugas, urusan warung, sampai ide yang muncul tiba-tiba. Cukup praktis, tapi WhatsApp memang tidak dirancang untuk mengelola catatan dan pengingat, dan informasinya makin sulit disortir seiring jumlahnya bertambah.'
			},
			solution: {
