...
export const projects: Project[] = [
	{
		...
		caseStudy: {
			problem: {
				en: 'While working on DevNote (Project 2), the use of AI in the development process became increasingly intensive as the project grew more complex. As a result, it became harder to maintain a complete understanding of the codebase and how its different parts were connected. At the same time, a broader problem became apparent among developers using AI: whenever a new session starts, the AI has to rebuild its context and understanding of the project again. Some AI coding tools already provide their own approaches to preserving context, such as CLAUDE.md in Claude and indexing in Cursor, but these solutions remain tied to their respective ecosystems. Combining these two problems led to the idea of building a tool that could understand and preserve codebase context in a reusable way, without depending on a specific AI agent or platform',
				id: 'Saat mengerjakan DevNote (Project 2), penggunaan AI dalam proses development mulai semakin intens seiring project yang semakin kompleks. Akibatnya, semakin sulit untuk mempertahankan pemahaman menyeluruh terhadap struktur dan hubungan antarbagian codebase. Di sisi lain, muncul masalah yang lebih umum di kalangan developer yang menggunakan AI: setiap kali memulai sesi baru, AI perlu membangun kembali konteks dan pemahaman terhadap project yang sedang dikerjakan. Beberapa AI coding tools memang sudah memiliki pendekatan untuk menyimpan konteks, seperti CLAUDE.md pada Claude dan indexing pada Cursor, tetapi solusi tersebut masih bergantung pada ekosistem masing-masing. Dari dua masalah tersebut, muncul kebutuhan untuk membuat satu tool yang dapat memahami dan menyimpan konteks codebase secara reusable, tanpa bergantung pada AI agent atau platform tertentu.'
			},
			solution: {
